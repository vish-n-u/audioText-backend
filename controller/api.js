...
const audioTranscription = async (req, res) => {
  try {
   ...
   await readFile(file.path).then((buffer) => {
      ...
      const durationSeconds = audioData.channelData[0].length / audioData.sampleRate;
  let doc =  await   UserDataModel.findOne({userId:req.body.uid})
   doc.usedTranscriptionTimeInMilliSec = String(Number(doc.usedTranscriptionTimeInMilliSec)+(durationSeconds*1000))
   data = await doc.save()
